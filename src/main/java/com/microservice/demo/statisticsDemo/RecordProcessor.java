...
public class RecordProcessor implements StatProcessor {
    ...
    public void process(String[] recordParts) throws ParseException {
        String traceId;
        ...
        if ("response".equals(recordType)) {
            ...
            //获取这条响应记录相应的请求记录中的tranceId
            matchingReqTraceId = traceId.substring(0, nonSeqLen) + df.format(Integer.valueOf(traceIdSeq).intValue() - Integer.valueOf(traceIdDiff).intValue());
            ...
            if (reqTimeStamp - currRecordData < sampleInterval * 1000) {
                delayStatData = summaryResult.get(currRecordData);
            } else {
                currRecordData = reqTimeStamp;
                delayStatData = new DelayItem(currRecordData);
                delayStatData.getTotalDelay().addAndGet(delay);
                summaryResult.put(currRecordData, delayStatData);
            }
    ...
    @Override
    public void process(String record) throws ParseException {
